...
{
    // минимум
    int mn = qFloor(16*bs/31);
    // ограничение минимального размера
    if (sz < mn) sz = mn;
    // максимум
    int mx = qFloor(31*bs/16);
...
void QAxisZoomSvc::showZoomWidget(QPoint evpos,int ax)
{
    ...
    int hw = qFloor(w/2);    // полуширину
    int h = zr->height();   // высоту
    int hh = qFloor(h/2);    // полувысоту
