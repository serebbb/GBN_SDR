...
int QwtChartZoom::meanLabelWidth(int xAx)
{
    ...
    if (lbwds->count() > 0) res = qFloor(res / lbwds->count());
...
bool QwtChartZoom::updateHorAxis(int xAx,int *hDiv)
{
    ...
    const int dw = 86;  // минимальное расстояние между метками //my 48
...
bool QwtChartZoom::updateVerAxis(int yAx,int *vDiv)
{
    ...
    int dv = qFloor(qwtp->canvas()->size().height()/(szlb.height()+8))/2; //my
...
{
    // если событие произошло для графика, то
    if (target == zoom->plot()  && zoom->isZoom)
...
void QMainZoomSvc::showZoomWidget(QRect zr)
{
    ...
    QRegion rs(2,2,dw-4,dh-4);  // прозрачная область
...
void QDragZoomSvc::showDragWidget(QPoint evpos)
{
    ...
    QRegion rs(2,2,ww-4,wh-4);  // прозрачная область
